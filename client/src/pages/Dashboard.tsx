...
const MetricCard = ({
  ...
  const getStatusColor = (
    ...
    if (isLowerBetter) {
      if (numValue <= numTarget)
        return "bg-gradient-to-br from-emerald-500/20 to-emerald-500/10 text-emerald-500 border-emerald-500/20";
      if (numValue <= numTarget * 1.1)
        return "bg-gradient-to-br from-yellow-500/20 to-yellow-500/10 text-yellow-500 border-yellow-500/20";
      return "bg-gradient-to-br from-red-500/20 to-red-500/10 text-red-500 border-red-500/20";
    } else {
      if (numValue >= numTarget)
        return "bg-gradient-to-br from-emerald-500/20 to-emerald-500/10 text-emerald-500 border-emerald-500/20";
      if (numValue >= numTarget * 0.9)
        return "bg-gradient-to-br from-yellow-500/20 to-yellow-500/10 text-yellow-500 border-yellow-500/20";
      return "bg-gradient-to-br from-red-500/20 to-red-500/10 text-red-500 border-red-500/20";
    }
  };
  ...
  return (
    <div
      className={`p-4 sm:p-6 rounded-xl transition-all duration-300 border ${statusColor}`}
    >
      ...
      <div className="space-y-1">
        <h3 className="text-sm sm:text-base font-medium opacity-90 line-clamp-2">
          {title}
        </h3>
        <div className="text-xl sm:text-2xl font-bold">{value}</div>
        <div className="text-xs sm:text-sm opacity-60">الهدف: {target}</div>
      </div>
    </div>
  );
};
      ...
      <div className="container mx-auto p-2 sm:p-4 md:p-6">
        <div className="space-y-4 md:space-y-6">
          ...
          <h2 className="text-lg sm:text-xl font-semibold">
            ...
            {currentPeriod === "weekly" ? "الأسبوعية" : "السنوية"}
          </h2>
...
const CustomerServiceStats = () => {
  ...
  const getColorByPercentage = (percentage: number): string => {
    if (percentage >= 75)
      return "bg-gradient-to-br from-emerald-500/20 to-emerald-500/10 text-emerald-500";
    if (percentage >= 50)
      return "bg-gradient-to-br from-yellow-500/20 to-yellow-500/10 text-yellow-500";
  ...
  const handleDeleteComment = async (indexToDelete: number) => {
    try {
      const commentToDelete = Array.isArray(maintenanceSatisfaction.comments)
        ? maintenanceSatisfaction.comments[indexToDelete]
        : null;
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <h2 className="text-lg sm:text-2xl font-bold">
        خدمة العملاء {currentPeriod === "weekly" ? "الأسبوعية" : "السنوية"}
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                      ...
                      {key === "complaints"
                        ...
                        : key === "contactRequests"
                          ...
                          : key === "maintenanceRequests"
                            ...
                            : key === "inquiries"
                              ? "استفسارات"
                              : key === "officeInterested"
                                ? "مهتمين مكاتب"
                                : key === "projectsInterested"
                                  ? "مهتمين مشاريع"
                                  : "عملاء مهتمين"}
                    </span>
        ...
        <Card className="border-0 bg-[#1a1c23] text-white">
          ...
          <CardContent className="pt-4">
            <div className="space-y-2">
              {Object.entries(customerServiceData.inquiries).map(
                ([key, value]) => (
                  <div
                    key={key}
                    className="flex justify-between items-center py-2 border-b border-gray-800 last:border-0"
              ...
              <div className="flex justify-between items-center pt-2 mt-2 border-t border-gray-800">
                ...
                <span className="text-base sm:text-lg font-bold text-white">
                  {Object.values(customerServiceData.inquiries).reduce(
                    (sum, val) => sum + val,
                    0,
                  )}
                </span>
        ...
        <Card className="border-0 bg-[#1a1c23] text-white">
          ...
          <CardContent className="pt-4">
            <div className="space-y-2">
              {Object.entries(customerServiceData.maintenance).map(
                ([key, value]) => (
                  <div
                    key={key}
                    className="flex justify-between items-center py-2 border-b border-gray-800 last:border-0"
              ...
              <div className="flex justify-between items-center pt-2 mt-2 border-t border-gray-800">
                ...
                <span className="text-base sm:text-lg font-bold text-white">
                  {Object.values(customerServiceData.maintenance).reduce(
                    (sum, val) => sum + val,
                    0,
                  )}
                </span>
            ...
            <div className="space-y-2">
              {Array.isArray(maintenanceSatisfaction.comments) ? (
                maintenanceSatisfaction.comments.map((comment, index) => (
                  <div
                    key={index}
                    className="group p-3 bg-[#252832] rounded-lg flex justify-between items-start"
                  >
