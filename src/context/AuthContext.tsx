...
const initializeDefaultAdmin = () => {
  ...
  const missingUsers = defaultUsers.filter(
    (defaultUser) =>
      !currentUsers.some(
        (currentUser) => currentUser.username === defaultUser.username,
      ),
  );
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // استعادة حالة تسجيل الدخول من localStorage
    const savedUser = localStorage.getItem("auth_current_user");
    if (savedUser) {
      try {
        const parsedUser = JSON.parse(savedUser);
        setUser(parsedUser);
        console.log("تم استعادة جلسة المستخدم:", parsedUser.username);
      } catch (error) {
